...
async function testNeonLocalBehavior() {
  ...
  globalThis.fetch = async (url, options) => {
    ...
    const hasNeonHeaders = Object.keys(options.headers).some((h) =>
      h.startsWith('X-Neon-'),
    );
    if (!hasNeonHeaders) {
  ...
  try {
    ...
    console.log(
      '🔍 useSecureWebSocket (auto-configured):',
    ...
    if (neonConfig.isNeonLocal !== true) {
      throw new Error(
        `Expected isNeonLocal to be true, got: ${neonConfig.isNeonLocal}`,
      );
    }

    if (neonConfig.useSecureWebSocket !== false) {
      throw new Error(
        `Expected useSecureWebSocket to be auto-configured to false, got: ${neonConfig.useSecureWebSocket}`,
      );
    }
    ...
    if (!endpoint.includes('http://127.0.0.1:5432/sql')) {
      throw new Error(
        `Expected http://127.0.0.1:5432/sql URL, got: ${endpoint}`,
      );
    }
    ...
    globalThis.fetch = async (url, options) => {
      ...
      if (options.headers['X-Neon-User'] !== 'testuser') {
        throw new Error(
          `Expected X-Neon-User to be 'testuser', got: ${options.headers['X-Neon-User']}`,
        );
      }

      if (options.headers['X-Neon-Password'] !== 'testpass') {
        throw new Error(
          `Expected X-Neon-Password to be 'testpass', got: ${options.headers['X-Neon-Password']}`,
        );
      }

      if (options.headers['X-Neon-Database'] !== 'testdb') {
        throw new Error(
          `Expected X-Neon-Database to be 'testdb', got: ${options.headers['X-Neon-Database']}`,
        );
      }
    ...
    const client = new Client(
      'postgresql://clientuser:clientpass@127.0.0.1:5432/clientdb',
    );
