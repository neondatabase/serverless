...
// export/index.ts
var NeonPool = class extends import_pg2.Pool {
  constructor(config) {
...
// src/index.ts
async function query(client, ctx) {
...
var src_default = {
  async fetch(request, env, ctx) {
    ...
    client = new NeonPool({ connectionString: env.NEON_DB_URL });
    ...
    client = new NeonClient(env.MY_DB_URL + "?sslmode=verify-full");
    client.neonConfig.wsProxy = (host, port) => `ws.manipulexity.com/v1?address=${host}:${port}`;
